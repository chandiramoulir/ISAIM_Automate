...
class ISAIM_auto:
    ...
    def DBcheck(self):
        ...
        self.cnxn = pymssql.connect("RAL-SYMPHONY-DB.itronhdc.com\ARTPILOT", "sa", "Meterguy1", "ItronEE")
        self.db_cursor = self.cnxn.cursor(as_dict=True)
        time.sleep(10)
        self.db_cursor.execute("select * from Meter where MeterID='AutoMeter_003'")
        try:
        ...
        if self.rows['MeterID'] == "AutoMeter_003":
